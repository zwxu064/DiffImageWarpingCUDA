...
if enable_load_matlab:
    ...
    if enable_debug:
        print('Check CUDA forward RAW: left diff: {:.8f}, right diff: {:.8f}; left count diff: {:.8f}, right count diff: {:.8f}; time: {:.6f}s.' \
              .format((img_left_cu.cpu() - img_left_gt).abs().max().numpy(),
    ...
    print('Check CUDA forward FINAL: left diff: {:.8f}, right diff: {:.8f}; left min: {:.8f}, max: {:8f}; time: {:.6f}s.' \
        .format((img_left_final_cu.cpu() - img_left_final_gt).abs().max().numpy(),
                (img_right_final_cu.cpu() - img_right_final_gt).abs().max().numpy(),
                img_left_final_cu.cpu().min(), img_left_final_cu.max(), duration))
else:
